...
export default function RallyDetail() {
  ...
  if (error) return <div style={{ background: '#1e2a3a', color: 'white', padding: '40px', textAlign: 'center' }}>Error: {error}</div>
  if (!rally) return <div style={{ background: '#1e2a3a', color: 'white', padding: '40px', textAlign: 'center' }}>Loading...</div>
    ...
    <div style={{ minHeight: '100vh', background: '#1e2a3a', color: 'white', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      <div style={{ padding: '40px 20px', maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ background: '#2d3e50', borderRadius: '16px', padding: '40px', border: '1px solid rgba(255, 255, 255, 0.05)', position: 'relative' }}>
          ...
          <button onClick={() => setShowDeleteConfirm(true)} style={{ position: 'absolute', top: '20px', right: '20px', background: 'rgba(255, 82, 82, 0.2)', color: '#FF5252', border: '1px solid #FF5252', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>
            Delete Rally
          </button>
        </div>
